...
- `POST /api/v1/auth/refresh` 刷新 access token（refresh token 轮换）
...
- `GET /api/v1/contacts/export.vcf` 导出 vCard 文件
- `GET /api/v1/audit` 查询审计日志
- `GET /api/v1/metrics` 查询接口调用与延迟统计
- `GET /health` 健康检查
...
- Token 使用 HS256 JWT 自实现，已支持 refresh token 轮换；生产环境建议改为成熟库并加入密钥轮换与黑名单缓存。
